function [k_el, damage_el, gausspoints_prop_mat_elem, nodes_prop_mat_elem, f_internal_el, f_external_el] = func_elstif_Nonlocintegral(lmn,lmncoord,dofs,Delastic,damage_mat_previousinc,alpha_val,beta_val,e_delta,dmax,n_hood,weights,damage_mat_it,IsM,IsProj)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if IsProj == 0
    ...
    for integ_point = 1:npoints
        ...
        [~, domega_dkappa] = func_mazarmodel_Nonlocintegral(e_star,alpha_val,beta_val,e_delta,dmax);


        % =============== NON LOCAL DAMAGE MATRIX CALCULATIONS ===============
        num   = 0;
        denom = 0;
        to    = sum(n_hood(4 * lmn + integ_point - 4,:) > 0);
        for i = 1:to
            ci    = ceil(n_hood(4 * lmn + integ_point - 4,i) / 4);
            num   = num + weights(4 * lmn + integ_point - 4,i) * damage_mat_it(ci,n_hood(4 * lmn + integ_point - 4,i) + 4 - (4 * ci));
            denom = denom + weights(4 * lmn + integ_point - 4,i);
        end
        
        % Compute nonlocal damage
        damage_nonlocal = num / denom;
   
        % Store the damage variable for each Gauss Point
        if IsM == 0
            damage_el(1,integ_point) = damage_nonlocal; 
        else
            damage_el(1,integ_point) = max(damage_mat_previousinc(1,integ_point),damage_nonlocal);
        end
...
%                    for jj = 1:2
%                        col = 2 * (ii - 1) + jj;
%                        dD_duj = -1 * domega_dkappa * (s(1) * B(1,col) + s(2) * B(2,col) + s(3) * B(3,col));
%                        for ki = 1:4
%                            for kj = 1:2
%                                kind = 2 * (ki - 1) + kj;
...
%         end
        ...
        Kint = KA; % + KB;
        k_el = k_el + (Kint* w(integ_point) * dt) ; % Size: 8x8 (1x1 * 8x3 * 3x3 * 3x8 * 1x1 * 1x1)
