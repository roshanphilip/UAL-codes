...
if IsProj == 0
    ...
    for lmn = 1:nelem
        ...
        for a = 1:nelnodes(lmn)
            ...
            for i = 1:ndof
                lmndof(i,a) = dofs(ndof*(connect_nds(a,lmn)-1)+i);
            end
        end
            ...
            for a = 1:nelnodes(lmn)
                for i = 1:ndof
                    for b = 1:nelnodes(lmn)
                        for k = 1:ndof
                            rw = ndof*(connect_nds(a,lmn)-1)+i;
                            cl = ndof*(connect_nds(b,lmn)-1)+k;
        ...
        if TangentID == 1
            for a = 1:nelnodes(lmn)
                for i = 1:ndof2
                    for b = 1:nelnodes(lmn)
                        for k = 1:ndof2
                            rw = ndof2*(connect_nds(a,lmn)-1)+i;
                            cl = ndof2*(connect_nds(b,lmn)-1)+k;
